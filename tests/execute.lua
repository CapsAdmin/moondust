...
local function generic_output(output, cb)
	local a = Assembler()
	cb(a, output)
	a:build("void (*)()")()
	return output[0]
...
local function generic_return(cb)
	local a = Assembler()
	cb(a)
	return a:build("uint64_t (*)()")()
end
...
check(
	...
	generic_output(
		...
		function(a, output)
			...
			a:movsd(
				r.xmm0,
				r({class = "simd", indirect = true, disp = memory.object_to_address(input + 0)})
			)
			a:movsd(
				r.xmm1,
				r({class = "simd", indirect = true, disp = memory.object_to_address(input + 1)})
			)
			a:movsd(
				r.xmm2,
				r({class = "simd", indirect = true, disp = memory.object_to_address(input + 2)})
			)
			...
			a:movsd(
				r({class = "simd", indirect = true, disp = memory.object_to_address(output)}),
				r.xmm0
			)
			a:ret()
		...
		"sqrt(x*x + y*y + z*z) should be TARGET"
	)
)
check(
	1337ull,
	"==",
	generic_return(function(a)
...
check(
	...
	"rax should be TARGET"
)
check(
	6,
	"==",
	generic_output(ffi.new("double[1]"), function(a, output)
		a:movsd(r.xmm0, r(memory.object_to_address(ffi.new("double[1]", 3))))
		a:mulsd(r.xmm0, r(memory.object_to_address(ffi.new("double[1]", 2))))
		a:movsd(r(memory.object_to_address(output)), r.xmm0)
		a:ret()
	end),
	"expected TARGET"
)
check(
	...
	generic_output(ffi.new("double[1]"), function(a, output)
		a:movsd(r.xmm0, r(memory.object_to_address(ffi.new("double[1]", 3))))
		a:addsd(r.xmm0, r(memory.object_to_address(ffi.new("double[1]", 2))))
		a:movsd(r(memory.object_to_address(output)), r.xmm0)
		a:ret()
	end),
	"expected TARGET"
...
check(
	5,
	"==",
	generic_output(ffi.new("int[1]", 2), function(a, output)
		a:mov(r.rdx, memory.object_to_address(output) - 1024)
		a:mov(r.rax, 3)
...
check(
	5,
	"==",
	generic_return(function(a, output)
...
do -- 2+5 memory
	...
	do
		a:mov(return_value, 3)
		a:add(r(memory.object_to_address(res)), return_value)
		a:ret()
	end
