...
test("mov reg to pointer", function()
	for _, val in ipairs(test_values) do
		...
		local asm = Assembler()
		asm:mov(R.rax, val)
		asm:mov(R({indirect = true, disp = memory.object_to_address(mem)}), R.rax)
		asm:ret()
		asm:build("void (*)(void)")()
...
test("mov pointer to reg", function()
	for _, val in ipairs(test_values) do
		...
		mem[0] = val
		local asm = Assembler()
		asm:mov(R.rax, R({indirect = true, disp = memory.object_to_address(mem)}))
		asm:ret()
		local result = asm:build("uint64_t (*)(void)")()
...
test("mov reg pointer roundtrip", function()
	for _, val in ipairs(test_values) do
		...
		asm:push(R.rbx)
		asm:mov(R.rbx, val)
		asm:mov(R({indirect = true, disp = memory.object_to_address(mem)}), R.rbx)
		asm:mov(R.rax, R({indirect = true, disp = memory.object_to_address(mem)}))
		asm:pop(R.rbx)
		asm:ret()
		local result = asm:build("uint64_t (*)(void)")()
